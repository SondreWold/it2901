	...
	calcMissingResources = (absentEmployees, base, employees, date) => {
		const missingResources = 
			absentEmployees
				...
				.filter(e => moment(e.date).format("YYYY-MM-DD") == moment(date).format("YYYY-MM-DD"))

		return missingResources.length;
	}

  render(){
  	const missingResources = this.calcMissingResources(this.props.absentEmployees,
  	 this.props.base, this.props.employees, this.props.date)
  	/*
